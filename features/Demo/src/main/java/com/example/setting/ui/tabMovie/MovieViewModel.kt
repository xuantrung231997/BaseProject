...
class MovieViewModel @Inject constructor(
    ...
    private val _loadMoreMovies = MutableLiveData<List<Movie>>()
    val loadMoreMovies: LiveData<List<Movie>> get() = _loadMoreMovies
    ...
    fun loadMoreMovieIfNeed() {
        ...
        if (resourceMovie is Resource.Success) {
            ...
            if (currentMoviePageIndex <= totalPage) {
                currentMoviePageIndex++
                repository.getMovieInfo(currentMoviePageIndex)
                    .onStart {
                        val loadingData = mutableListOf(Movie(LOADING_MOVIE_ID))
                        _loadMoreMovies.value = loadingData
                    }.map {
                        val moviesResponse = it.results
                        _loadMoreMovies.value = moviesResponse
